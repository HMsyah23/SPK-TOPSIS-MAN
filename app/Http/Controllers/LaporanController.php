...
use PDF;
use App\Kriteria,App\Subkriteria,App\NilaiCalon;
...
class LaporanController extends Controller
{
    public function laporanRanking(){
        ...
        foreach ($data as $key => $value) {
            ...
            $value['foto'] = json_decode($value->calon->berkas)->ktp;
        ...
        $ranks = $data->sortByDesc('preferensi');
        ...
        $pdf = PDF::setOptions(['isHtml5ParserEnabled' => true, 'isRemoteEnabled' => true])->loadView('laporan.ranking',compact('ranks','base64'))
        // ->setOptions(['isPhpEnabled' => true])
        ->setPaper('a4', 'portrait');
        return $pdf->stream();
	}

    public function laporanPelamar($id)
	{
		$calonPenerima = CalonPenerima::find($id);
        $path = 'images/logo.jpg';
        $type = pathinfo($path, PATHINFO_EXTENSION);
        $data = file_get_contents($path);
        set_time_limit(300);
        $base64 = 'data:image/' . $type . ';base64,' . base64_encode($data);
		$pdf = PDF::setOptions(['isHtml5ParserEnabled' => true, 'isRemoteEnabled' => true])->loadView('laporan.calonPenerima',compact('calonPenerima','base64'))
        // ->setOptions(['isPhpEnabled' => true])
        ->setPaper('a4', 'portrait');
        return $pdf->stream();
	}
