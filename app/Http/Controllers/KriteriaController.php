...
class KriteriaController extends Controller
{
    ...
    public function cek(Request $r){
        // dd($r->all());
        $id = CalonPenerima::where('nik',$r->nomor)->first();
        if($id == null){
            $id = CalonPenerima::where('no_ktp',$r->nomor)->first();
        }
        if($id == null){
            Session::flash('error', "NIK/No KK <strong>".$r->nomor."</strong> Tidak Ditemukan");
            return redirect()->route('awal');
        }
        return redirect()->route('ketemu',$id);
    }
    ...
    public function ketemu($id){
        ...
        return view('ketemu',compact('id','rank'));
    }
}
